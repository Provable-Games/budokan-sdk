...
> **Update — the per-tx Mini App was removed.** Auth now uses only the Cartridge slot-pattern callback (a regular browser redirect, not an in-Telegram `web_app`). The per-tx approval problem is instead solved with **session spending limits**: `/connect` authorizes `approve` on the common entry-fee tokens up to a per-token cap (`policies.ts` + `catalog/tokens.ts`), so paid `/enter` approves the exact fee and enters in one in-session multicall — no popup. Fees above the cap, unknown fee tokens, and `/add_prize` deeplink to budokan.gg. The `miniapp/` tree and `/api/tx/*` routes no longer exist. Sections below describing the Mini App / per-tx handshake are retained as historical design rationale. Note also: the read commands are dispatched directly from `server/src/telegram.ts` (`/tournaments`, `/my_tournaments`, `/leaderboard`) — there is no `server/src/commands/readonly.ts`, and `/follow` / `/following` / `/prizes` / `/tournament` exist only in the read-only `.mjs` bot, not here.
